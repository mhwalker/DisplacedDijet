...
class DJ(object) :
    ...
    def common(self) :
	    ...
	    print module
            self.process.load('DisplacedDijet.DisplacedJetAnlzr.DJ_%s_cfi'%module)
    ...
    def JetSelector(self):
        self.process.load("DisplacedDijet/DisplacedJetAnlzr/JetSelector_cfi")
        return (cms.Sequence(self.process.trackerPatJets))

    def V0(self):
        self.process.load("DisplacedDijet/DisplacedJetAnlzr/generalV0Candidates_cfi")
        return (cms.Sequence(self.process.myV0Candidates))

    def MCWeights(self):
        if self.options.signal:
            self.process.load("DisplacedDijet/DisplacedJetAnlzr/PdfWeightProducer_cfi")
